...
if __name__ == '__main__':
    for setting in ["within-apps", "across-apps"]:
        ...
        filename = f'{base_path}/results/rq2/rq2-ALT-{setting}-webembed.csv'
        ...
        for feature in ['content_tags']:
                for app in APPS:
                    print(f'{app} - {baseline} - {feature}')
                    ss = ss[ss['appname'] == app]
                    cluster_file_name = f'{base_path}/dataset/clusters/{app}.json'

                    pred_file =  f'{base_path}/resources/csv_results_table/SS_as_distance_matrix_{setting}-{app}-{baseline}-{feature}.csv'
